...
std::string GDrawSomething(GCanvas* canvas, GISize dim) {
    ...
    // GColor rectColor = GColor::RGBA(0,1,0,1);
    // GColor rectColor2 = GColor::RGBA(0,0,1,0.8);
    // GRect rect = GRect::LTRB(100,50,230,175);
    // GRect rect2 = GRect::LTRB(0,0,5,100);

    // canvas->fillRect(rect, rectColor);
    // canvas->fillRect(rect2, rectColor2);
    ...
    // GPaint paintt;
    // paintt.setRGBA(1, 0, 0, 1);
    // paintt.setBlendMode(GBlendMode::kDstOut);

    // GPoint arr[5] = {p1,p2,p3,p4,p5};

    // canvas->drawConvexPolygon(arr, 5, paintt);
    ...
    // canvas->drawRect(GRect::XYWH(20, 20, 100, 100), paintor);
    ...
    // canvas->drawConvexPolygon(points, 4, paintor);

    GPoint one, two, three;
    one.x = 20;
    one.y = 235;
    two.x = 235;
    two.y = 235;
    three.x = 127;
    three.y = 20;
    GPoint points[3] = {one, two, three};

    int indices[3] = {0, 1, 2};

    GColor yellow = GColor::RGBA(1, 236.0/255.0, 0, 1);
    GColor green = GColor::RGBA(0, 135.0/255.0, 34.0/255.0, 1);

    GColor colors[] = {green, green, yellow};

    canvas->drawMesh(points, colors, NULL, 1, indices, GPaint());

    return "Triangle of Life"; //name of artwork
}